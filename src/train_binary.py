...
BEST_MODEL_PATH = config.CHECKPOINT_DIR / "best_binary_tumor_gate.pth"
LAST_MODEL_PATH = config.CHECKPOINT_DIR / "last_binary_tumor_gate.pth"
CURVES_PATH = config.OUTPUT_DIR / "binary_training_curves.png"
...
def compute_class_weights(
    ...
    targets = [int(target) for target in train_dataset.targets]
    counts = Counter(targets)
    total = len(targets)
    weights = [
        total / (len(class_names) * counts[class_idx])
    ...
    print("Binary CrossEntropyLoss class weights:")
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "task": "binary_tumor_vs_notumor_gate",
    }
...
def main() -> int:
    ...
    train_loader, val_loader, _, class_names = create_binary_dataloaders()
    ...
    for epoch in range(1, config.NUM_EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"Saved new best binary checkpoint: {BEST_MODEL_PATH}")
        else:
    ...
    print(f"Last binary checkpoint: {LAST_MODEL_PATH}")
    print(f"Best binary checkpoint: {BEST_MODEL_PATH}")
    print(f"Binary training curves: {CURVES_PATH}")
    return 0
